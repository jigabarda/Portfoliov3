...
const items = [
  {
    ...
    image:
      "https://images.unsplash.com/photo-1506748686214-e9df14d4d9d0?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=MnwzNjUyOXwwfDF8c2VhcmNofDJ8fGJsb2J8ZW58MHx8fHwxNjg5NTY1NzA3&ixlib=rb-4.0.3&q=80&w=1080",
...
const Projects = () => {
  ...
  return (
    <div className="h-screen bg-red-300 panel transition-colors duration-500">
      <h2 className="text-center font-semibold font-montserrat text-5xl pt-8 pb-4">
        Projects
      </h2>
      ...
      <div className="flex justify-center gap-4 mb-4">
        {categories.map((cat) => (
          <button
            key={cat}
            className={`no-box-shadow cursor-pointer group relative px-2 py-2 font-semibold border-none bg-transparent text-red-500 transition-colors duration-200 focus:outline-none
              ${
      ...
      <div className="grid grid-cols-3 px-50 grid-rows-2 gap-12">
        {filteredItems.map((item, idx) => (
          <div
            key={idx}
            className="relative bg-white rounded-lg shadow flex flex-col items-center transition-colors duration-500"
          >
            <Image
              src={item.image}
              alt={item.title}
